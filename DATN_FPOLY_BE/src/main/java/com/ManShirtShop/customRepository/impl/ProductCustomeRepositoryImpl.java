...
public class ProductCustomeRepositoryImpl implements ProductCustomeRepository {
    ...
    public List<SearchProductClient> getAllClients(ProductFilterClientRequest filter) {
        ...
        if (filter.getIdDiscount() > 0 || filter.getDiscount() > 0) {
            ...
            if (filter.getIdDiscount() > 0) {
                where.append(" AND (product_discount.status = 0) AND (discount.status = 0) AND ( discount.id = :discount ) AND (DATE(discount.end_date) >= DATE(now())) AND (DATE(discount.start_date) <= DATE(now()))");
        ...
        String qryFull = "";
        if (filter.getFillter() == 0) {
            qryFull = "SELECT prd.id,prd.status,prd.name,prd.description,prd.price,prd.weight,prd.code,prd.laster,\n" +
        ...
        } else if (filter.getFillter() == 1) { // discount
            qryFull = "SELECT prd.id,prd.status,prd.name,prd.description,prd.price,prd.weight,prd.code,prd.laster,\n" +
                    "CASE WHEN prd.rating IS NULL THEN 5 ELSE prd.rating END AS 'rating',\n" +
                    "CASE WHEN prd.soluongdaban IS NULL THEN 0 ELSE prd.soluongdaban END AS 'soluongdaban',\n" +
                    "CASE WHEN dis.percent IS NULL THEN 0 ELSE dis.percent END AS 'discount',\n" +
                    ...
                    "where product.id IN (" + qry + ")GROUP BY product.id HAVING product.id) as cus ON product.id = cus.id\n" +
                    "WHERE product.status = 0 and product.id IN (" + qry + ")\n" +
                    "GROUP BY product.id HAVING product.id) as prd\n" +
                    "left join(SELECT product_discount.product_id, product_discount.percent, discount.start_date,discount.end_date\n" +
                    "FROM defaultdb.product_discount\n" +
                    "left JOIN defaultdb.discount ON product_discount.discount_id = discount.id\n" +
                    "where product_discount.status = 0 and discount.status = 0 and  DATE(discount.start_date) <= NOW()\n" +
                    "and DATE(discount.end_date) >= NOW() order by product_discount.percent) as dis on prd.id = dis.product_id where dis.percent is not null order by dis.percent desc";
        } else if (filter.getFillter() == 2) { //new product
            qryFull = "SELECT prd.id,prd.status,prd.name,prd.description,prd.price,prd.weight,prd.code,prd.laster,\n" +
                    "CASE WHEN prd.rating IS NULL THEN 5 ELSE prd.rating END AS 'rating',\n" +
                    "CASE WHEN prd.soluongdaban IS NULL THEN 0 ELSE prd.soluongdaban END AS 'soluongdaban',\n" +
                    "CASE WHEN dis.percent IS NULL THEN 0 ELSE dis.percent END AS 'discount',\n" +
                    ...
                    "INNER JOIN defaultdb.product_detail ON order_details.product_detail_id = product_detail.id\n" +
                    "INNER JOIN defaultdb.product ON product_detail.product_id = product.id \n" +
                    "where product.id and product.id IN (" + qry + ") and DATEDIFF(NOW(), product.create_time) <= 30 GROUP BY product.id HAVING product.id order by product.create_time desc) as cus ON product.id = cus.id\n" +
                    "WHERE product.status = 0 and product.id IN (" + qry + ") and DATEDIFF(NOW(), product.create_time) <= 30\n" +
                    "GROUP BY product.id HAVING product.id order by product.create_time desc) as prd\n" +
                    "left join(SELECT  product_discount.product_id, product_discount.percent, discount.start_date,discount.end_date\n" +
                    "FROM defaultdb.product_discount\n" +
                    "left JOIN defaultdb.discount ON product_discount.discount_id = discount.id\n" +
                    "where product_discount.status = 0 and discount.status = 0 and  DATE(discount.start_date) <= NOW()\n" +
                    "and DATE(discount.end_date) >= NOW()) as dis on prd.id = dis.product_id;";
        } else if (filter.getFillter() == 3) { // top order
            qryFull = "SELECT prd.id,prd.status,prd.name,prd.description,prd.price,prd.weight,prd.code,prd.laster,\n" +
                    "CASE WHEN prd.rating IS NULL THEN 5 ELSE prd.rating END AS 'rating',\n" +
                    "CASE WHEN prd.soluongdaban IS NULL THEN 0 ELSE prd.soluongdaban END AS 'soluongdaban',\n" +
                    "CASE WHEN dis.percent IS NULL THEN 0 ELSE dis.percent END AS 'discount',\n" +
                    ...
                    "INNER JOIN defaultdb.product_detail ON order_details.product_detail_id = product_detail.id\n" +
                    "INNER JOIN defaultdb.product ON product_detail.product_id = product.id \n" +
                    "where product.id and  product.id IN (" + qry + ") GROUP BY product.id HAVING product.id) as cus on product.id = cus.id)\n" +
                    "as cus ON product.id = cus.id\n" +
                    "WHERE product.status = 0 and product.id IN (" + qry + ") and cus.id\n" +
                    "GROUP BY product.id HAVING product.id) as prd\n" +
                    "left join(SELECT  product_discount.product_id, product_discount.percent, discount.start_date,discount.end_date\n" +
                    "FROM defaultdb.product_discount\n" +
                    "left JOIN defaultdb.discount ON product_discount.discount_id = discount.id\n" +
                    "where product_discount.status = 0 and discount.status = 0 and  DATE(discount.start_date) <= NOW()\n" +
                    "and DATE(discount.end_date) >= NOW()) as dis on prd.id = dis.product_id order by prd.soluongdaban desc";
        }
