...
public class GhnServiceImpl implements GhnService {
    ...
    @Transactional
    @Override
    public String ghncreateExchange(Integer id) {
        List<String> lstEmail = new ArrayList<>();
        Optional<Exchange> ex = exchangeRepository.findById(id);
        if (!ex.isPresent()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy Order");
        }
        ex.get().setStatus(OrderContant.STATUS_DANG_GIAO);
        ex.get().setUpdateTime(Timestamp.from(Instant.now()));
        String fullName = employeeRepository.getFullNameByEmail(EmailUser.getEmailUser());
        ex.get().setUpdateBy(fullName);
        String url = "https://dev-online-gateway.ghn.vn/shiip/public-api/v2/shipping-order/create";
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("ShopId", shopid);
        headers.set("Token", token);
        if (!(ex.get().getCustomer().getEmail() == null) || !ex.get().getCustomer().getEmail().isBlank()) {
            lstEmail.add(ex.get().getCustomer().getEmail());
        }
        Map<String, Object> requestBody = new HashMap<>();
        requestBody.put("payment_type_id", 1);
        requestBody.put("note", ex.get().getNote());
        requestBody.put("required_note", "CHOXEMHANGKHONGTHU");
        requestBody.put("client_order_code", "");
        requestBody.put("to_name", ex.get().getShipName());
        requestBody.put("to_phone", ex.get().getShipPhone());
        requestBody.put("to_address", ex.get().getShipAddress());
        requestBody.put("to_ward_code", ex.get().getIdWard());
        requestBody.put("to_district_id", ex.get().getIdDistrict());
        if (ex.get().getStatusPay() == OrderContant.CHUA_THANH_TOAN) {
            requestBody.put("cod_amount", (int) ex.get().getTotal());
        } else {
            requestBody.put("cod_amount", 0);
        }
        if (!ex.get().getNote().isBlank() || !(ex.get().getNote() == null) || !ex.get().getNote().equals("")) {
            requestBody.put("content", ex.get().getNote());
        } else {
            requestBody.put("content", "Khách hàng không chú thích");
        }
        requestBody.put("content", ex.get().getNote());
        ...
        requestBody.put("service_id", service);
        List<Map<String, Object>> items = new ArrayList<>();
        for (ExchangeDetail x : ex.get().getExchangeDetail()) {
            Map<String, Object> item = new HashMap<>();
            item.put("name", x.getProductDetail().getProduct().getName()
                    + " "
                    + x.getProductDetail().getColor().getName()
        ...
        try {
            ...
            exchange.setCodeGhn(get(json, "order_code"));
