...
import com.ManShirtShop.dto.Statistic.OrderReturnDetailResponse;
...
import com.ManShirtShop.dto.Statistic.Order.CountOrderResponse;
...
public class StatisticServiceImpl implements StatisticService{
    ...
    public Double getTotalBy(List<Object> resultList) {
        ...
        for (Object result : resultList) {
            if (result instanceof Object[]) {
                ...
                Double total = orderTotal - returnTotal; 
                listD.add(total);
            }
        }
        Double sum = 0.0;
    ...
    public Long getTotalProduct(){
        ...
        for(ProductDto productDto : resultList){
            Long total = productDto.getQuantityOrder()-productDto.getQuantityReturn();
    ...
    public List<TopProductResponse> getProductByDate(){ 
        ...
        for (ProductDto productDto : resultList) {
            ...
            topProduct.setQuantity(productDto.getQuantityOrder()-productDto.getQuantityReturn());
            topProduct.setTotal(productDto.getTotalOrder()-productDto.getTotalReturn());
    ...
    public List<TopCustomerResponse> getTopCustomer(){
        ...
        for (CustomerDto productDto : resultList) {
            ...
            topProduct.setCountProduct(productDto.getQuantityOrder()-productDto.getQuantityReturn());
            listProduct.add(topProduct);
        }
    ...
    public TotalByDateResponse getTotalByDate(StatisticRequest filter) {
        ...
        for (TotalDto result : resultList) {
            ...
            total = total + result.getTotalOrder() - result.getTotalReturn();
    ...
    public ProductDetailCResponse getProductDetailByCustomer(Integer id) {
        ...
        for (ProductDetailDto productDetailDto : resulList) {
            ...
            productDetail2.setCountProductDetail(productDetailDto.getQuantityOrder()-productDetailDto.getQuantityReturn());
        ...
        for(int i = 1; i <= 12; i++){
            ...
            for (OrderDto orderDto : listD) {
                if(orderDto.getMonth() == i){
                    ...
                    Double totalMoney = orderDto.getTotalOrder() - orderDto.getTotalReturn();
            ...
            for (ProductMonthDto productM : listP) {
                if(productM.getMonth() == i){
                    Long totalProduct = productM.getQuantityOrder() - productM.getQuantityReturn();
    ...
    public List<TotalMonthResponse> getTotalByQuarter(Integer year) {
        ...
        for(int i = 1; i <= 12; i++){
            ...
            //     if(orderDto.getMonth() == i){
            //         Double freight = orderDto.getTotalExchange() - orderDto.getExchange();
            //         Double totalMoney = orderDto.getTotalOrder() + orderDto.getTotalExchange();
            //         total = total + totalMoney;
            //     }
            // }
            // totalMonthResponse.setTotal(total);
    ...
    @Override
    public List<CountOrderResponse> countOrderByMonth(Integer year) {
        List<CountOrderResponse> list = new ArrayList<>();
        List<CountorderDto> listD = orderRepository.getReturnSuccessByMonth(year);
        List<CountorderDto> listP = orderRepository.getOrderCancelByMonth(year);
        List<CountorderDto> listO = orderRepository.getOrderSuccessByMonth(year);
        for(int i = 1; i <= 12; i++){
            Integer totalD = 0;
            Integer totalP = 0;
            Integer totalO = 0;
            CountOrderResponse totalMonthResponse = new CountOrderResponse();
            totalMonthResponse.setMonth(i);
            for (CountorderDto count : listO) {
                if(count.getMonth() == i){
                    totalO = totalO + count.getCount();
            ...
            totalMonthResponse.setOrderSuccess(totalO);
            for (CountorderDto count : listP) {
                if(count.getMonth() == i){
                    totalP = totalP + count.getCount();
                }
            }
            totalMonthResponse.setOrderCancel(totalP);
            for (CountorderDto count : listD) {
                if(count.getMonth() == i){
                    totalD = totalD + count.getCount();
                }
            }
            totalMonthResponse.setReturnSuccess(totalD);
            list.add(totalMonthResponse);
        }
        return list;
    }
    @Override
    public OrderReturnDetailResponse getByCode(String code){
        OrderReturnDetailResponse orderReturnDetailResponse = new OrderReturnDetailResponse();
        orderReturnDetailResponse.setOrderResponeAdmin(orderServiceAdmin.findByCode(code));
        String returnCode = orderRepository.getCodeReturn(code);
        
        if(returnCode != null){
            orderReturnDetailResponse.setReturnResponseAdminGetAll(returnService.findReturnAndDetailByCode(returnCode));
        }
