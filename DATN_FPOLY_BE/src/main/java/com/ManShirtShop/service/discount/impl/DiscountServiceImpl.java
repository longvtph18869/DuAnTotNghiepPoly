...
public class DiscountServiceImpl implements DiscountService {
    ...
    public List<DiscountResponse> getAll() {
        ...
        List<DiscountResponse> list = ObjectMapperUtils.mapAll(geListDb, DiscountResponse.class);
        for (DiscountResponse x: list) {
           x = getTrangThai(x);
        }
        return list;
    }
    public DiscountResponse getTrangThai(DiscountResponse dis){
        LocalDate start = dis.getStartDate().toLocalDate();
        LocalDate end = dis.getEndDate().toLocalDate();
        LocalDate today = LocalDate.now();
//        System.out.println(start);
//        System.out.println(end);
//        System.out.println(today);
        if (!today.isAfter(start)){
            dis.setTrangThai("Chưa đến ngày bắt đầu");
        } else if (today.isAfter(start)&&!today.isAfter(end)) {
            dis.setTrangThai("Đang hoạt động");
        }else if (!today.isAfter(start)&&!today.isAfter(end)) {
            dis.setTrangThai("Đã kết thúc");
        }
        return dis;
    }
    @Override
