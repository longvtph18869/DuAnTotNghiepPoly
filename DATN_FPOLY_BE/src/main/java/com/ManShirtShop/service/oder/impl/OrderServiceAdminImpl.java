...
public class OrderServiceAdminImpl implements OrderServiceAdmin {
    ...
    public List<OrderAllAdminByStatus> findByStatus(Integer status) {
        ...
        for (IOrderAllAdmin x : oderRepository.getAllByAdmin(status)) {
            ...
            Optional<String> codeReturn = returnRepository.getCodeByIdOrder(x.getId());
            if (codeReturn.isPresent()){
                y.setCheckReturn(true);
                y.setCodeReturn(codeReturn.get());
            }else {
                y.setCheckReturn(false);
                y.setCodeReturn(null);
            }
            response.add(y);
        }
        return response;
        ...
        try {
            ...
            for (Order x : lst) {
                ...
                for (OrderDetail pDetail : x.getOrderDetail()) {
                    ...
                    Integer soluong = p.getQuantity() - pDetail.getQuantity();
                    p.setQuantity(soluong);
                    if (soluong<0){
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Số lượng vượt quá đơn còn dư");
                    }
                    lDetails.add(p);
                }
                if (x.getPaymentType().equals("VNPAY")&&x.getStatusPay() == OrderContant.CHUA_THANH_TOAN){
                    x.setPaymentType("COD");
                }
